...
def gen_cli():
    ...
    This group provides commands for generating large files with random data
...
def csv(col, row, size):
    ...
    :param col: The number of columns in the CSV file.
    :param row: The number of rows in the CSV file (optional if size is specified).
    :param size: The target file size (optional if row count is specified).
    """
...
def json(size):
    ...
    :param size: The target file size for the JSON file.
    """
    try:
        target_file_size = convert_size_to_bytes(size)
    except ValueError as e:
